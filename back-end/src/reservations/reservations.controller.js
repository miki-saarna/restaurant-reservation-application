...
async function reservationExists(req, res, next) {
  ...
  try {
    const reservationFound = await service(req.app.get('db')).read(reservationId)
    if (!reservationFound) {
...
async function list(req, res) {
  ...
  if (reservation_id) {
    const data = await service(req.app.get('db')).readByReservationId(reservation_id)
    res.json({ data })
  } else if (date) {
    const data = await service(req.app.get('db')).readByDate(date)
    res.json({ data })
  } else if (mobile_number) {
    const data = await service(req.app.get('db')).readByNumber(mobile_number);
    res.json({ data })
  } else {
...
async function create(req, res) {
  const { data } = req.body;
  const newReservation = await service(req.app.get('db')).create(data);
  res.status(201).json({ data: newReservation })
}

async function edit(req, res) {
  const { data } = req.body;
  const edittedReservation = await service(req.app.get('db')).edit(data)
  res.json({ data: edittedReservation})
}

async function destroy(req, res) {
  const reservationToDelete = res.locals.reservationId;
  await service(req.app.get('db')).destroy(reservationToDelete);
  res.sendStatus(204);
}
...
async function updateStatus(req, res, next) {
  ...
  const data = await service(req.app.get('db')).updateStatus(reservation_id, newStatus);
  res.json({ data })
}
